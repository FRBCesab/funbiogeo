...
filename <- system.file("extdata", "woodiv_raw_data.csv", package = "funbiogeo")
...
test_that("fb_format_site_locations() errors with wrong input type", {
  ...
  expect_error(
    fb_format_site_locations(all_data, "site", "longitude", c("latitude", "y")),
    "Argument 'latitude' must be a character of length 1 (column name)",
    fixed = TRUE
  )
...
test_that("fb_format_site_locations() works with valid input", {
  ...
  expect_equal(sf::st_coordinates(site_locations)[1, 1][[1]], 1755000)
  ...
  expect_equal(nrow(site_locations), 695L)
})
