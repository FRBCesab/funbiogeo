...
test_that("fb_format_sites_locations() errors with missing input", {
  expect_error(
...
test_that("fb_format_sites_locations() errors with columns missing in input", {
...
test_that("fb_format_sites_locations() errors with wrong input type", {
  ...
  # Argument crs
  expect_error(
    fb_format_sites_locations(all_data, "site", "longitude", "latitude", "bla"),
    "Argument 'crs' should be valid CRS or coercible to one with sf::st_crs()",
    fixed = TRUE
  )
  
  expect_error(
    fb_format_sites_locations(all_data, "site", "longitude", "latitude", 4.2),
    "Argument 'crs' should be valid CRS or coercible to one with sf::st_crs()",
    fixed = TRUE
  )
...
# Test: Valid data -------------------------------------------------------------
test_that("fb_format_sites_locations() works with valid input", {
