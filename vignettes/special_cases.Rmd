...
**Note**: the only two functions of `funbiogeo` that won't work with categorical traits are `fb_cwm()` which computes an abundance-weighted trait average, and `fb_plot_trait_correlation()` which displays trait-trait correlations.
...
We will now convert the sites to points by taking the centroid of sites and using `fb_map_*()` functions to see how it will affect their outputs:
...
However, `funbiogeo` can handle sites of any shape. To display line-shaped sites, we will cluster them into groups of site lines and use the same function.
...
Similar to the [upscaling vignette](vignettes/upscaling.Rmd), the map functions can also accommodate larger polygons, for example by aggregating sites per country.
...
Have fun with `funbiogeo` and if you have a question, an issue, or a suggestion, make sure to fill a report [on GitHub](https://github.com/FRBCesab/funbiogeo/issues/new).
