...
upscaled_richness <- fb_aggregate_site_data(
  site_locations = site_locations[ , 1, drop = FALSE],
...
finer_map <- ggplot(single_species) +
  geom_sf(aes(fill = as.factor(CGRA))) +
  labs(fill = "Presence of CGRA", title = "Original resolution (10 x 10 km)")
...
agg_fric <- fb_aggregate_site_data(site_locations[ , 1, drop = FALSE], 
                                   site_fric, 
                                   coarser_grid)
