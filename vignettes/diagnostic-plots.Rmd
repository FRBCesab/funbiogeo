...
### Plot Trait Coverage for All Sites `fb_plot_site_traits_completeness()`
...
### Plot Trait Coverage per Species `fb_plot_species_traits_completeness()`

To select appropriate traits, we can visualize the trait coverage per species
per trait. This is done by `fb_plot_species_traits_completeness()` which takes
has `species_traits`, the species by traits `data.frame`, for sole argument.
...
species, it is blue if the trait is known and red if it is missing.
...
### Display Trait Combinations `fb_plot_trait_combination_frequencies()`

Looking at the coverage for each species continuously maybe impractical or
difficult. For example, when trying to display thousands of species or when the
trait coverage varies widely across trait. One way to reduce the size of the
analyze (and thus visualized) dataset is instead to count at which frequency
...
blue it means that the trait is present and when it is red it means the trait is
...
### Show Trait Correlations `fb_plot_trait_correlation()`
...
To be the most flexible possible, `funbiogeo` provides a function to mapping
...
additional column, the third and last required argument is `selected_col` which
should be the name of column provided in `site_data` that is going to be used as
...
environmental layers for example. The function will display the raster in
provided projection.
