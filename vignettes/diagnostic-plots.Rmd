...
metrics in other packages. This vignette explains in details all
plotting functions available in `funbiogeo`, how to use them and how to
interpret them.

Some of these plotting functions can deal with species categorization, i.e. 
displaying information by each category of species (family, order, endemism status, etc.).
...
The function outputs a dotchart. The number of sites is indicated at the 
...
prevalence. With this figure we see that 82.6% of the species
...
Let's say we want to see the species present at at least 40% of the sites:
...
another. However, for functional biogeography analyses, traits correlations maybe
...
The function `fb_map_site_traits_completeness()` is a specific function that 
maps the trait coverage (proportion of
