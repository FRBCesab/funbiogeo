...
of the species (in rows) (`species_traits` dataset in `funbiogeo`);
...
(`site_species` dataset in `funbiogeo`);
- the **site x locations** object, which describes the physical location of
sites through an `sf` object (`site_locations` dataset in `funbiogeo`).
...
In the following sections, we'll describe in detail these three provided
...
The package `funbiogeo` comes with the example dataset on Conifer tree species of Western European Mediterranean regions
`site_species`.
Let's look at it:
