...
contain the trait values for all species. Note that the species names are 
...
the location of the 1,505 sites (grid cells of 0.5° x 0.5° resolution).

```{r preview-sites-locs}
head(site_locations, 10)
```
...
We can use the function `fb_filter_species_by_traits_coverage()` with
