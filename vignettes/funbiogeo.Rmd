...
- the **site x locations** dataset (example dataset: `site_locs`)
...
The package `funbiogeo` comes with the example dataset `site_locs`.


```{r 'load-sites-locs'}
## Load site x locations example dataset ----
data("site_locs", package = "funbiogeo")
```

```{r 'preview-sites-locs', echo = FALSE}
knitr::kable(head(site_locs, 10), 
...
sites_env <- fb_get_environment(site_locs, layers)
...
ras_richness <- fb_aggregate_site_data(site_locations = site_locs, 
                                 site_data       = rich_env, 
                                 agg_grid       = tavg)
