...
check_site_species <- function(site_species) {
  ...
  if (!is.data.frame(site_species)) {
    stop("The site x species object must be a data.frame", call. = FALSE)
  }

  if (0 %in% dim(site_species)) {
    stop(
      "The site x species object should have at least one row and ",
      "one column",
      call. = FALSE
    )
  }
...
check_species_traits <- function(species_traits) {
  ...
  if (!is.data.frame(species_traits)) {
    stop("The species x traits object must be a data.frame", call. = FALSE)
  }
...
check_site_locations <- function(site_locations) {
  ...
  if (!inherits(site_locations, "sf")) {
    stop("The site x locations object must be an 'sf' object", call. = FALSE)
  }
...
check_threshold_proportion <- function(
  ...
  if (!is.numeric(threshold)) {
    stop(
      ...
      "' (",
      type,
      " coverage proportion) must be numeric",
      call. = FALSE
    )
  }
