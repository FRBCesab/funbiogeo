...
fb_plot_distribution_site_trait_coverage <- function(
  ...
  avg_coverage <- lapply(
    ...
    function(single_category) {
      site_avg_coverage <- by(
        single_category,
        list(coverage_name = single_category[["coverage_name"]]),
  ...
  grand_avg_coverage <- utils::stack(
    by(avg_coverage, avg_coverage$coverage_name, function(x) {
      mean(x$avg_coverage, na.rm = TRUE)
    })
  )
  ...
  if (!is_ggridges_installed()) {
    stop(
      "This function requires 'ggridges' to work\n",
  ...
  ggplot2::ggplot(
    ...
    ggplot2::scale_x_continuous(
      "Average Trait Coverage per Site",
