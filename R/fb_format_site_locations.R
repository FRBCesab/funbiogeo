...
#'   https://r.geocompx.org/spatial-class.html?q=CRS#crs-intro).
...
fb_format_site_locations <- function(
  ...
  if (!(site %in% colnames(data))) {
    stop(paste0("The column '", site, "' is absent from 'data'"), call. = FALSE)
  }
  ...
  if (!(longitude %in% colnames(data))) {
    stop(
      paste0("The column '", longitude, "' is absent from 'data'"),
      call. = FALSE
    )
  }
  ...
  if (!is.numeric(data[, longitude])) {
    stop(
      paste0("The column '", longitude, "' is must be a numeric"),
      call. = FALSE
    )
  }
  ...
  if (!(latitude %in% colnames(data))) {
    stop(
      paste0("The column '", latitude, "' is absent from 'data'"),
      call. = FALSE
    )
  }
  ...
  if (!is.numeric(data[, latitude])) {
    stop(
      paste0("The column '", latitude, "' is must be a numeric"),
      call. = FALSE
    )
  }
