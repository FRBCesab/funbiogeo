...
fb_plot_species_traits_completeness <- function(
  ...
  all_traits_subset <- lapply(
    ...
    function(x) {
      ...
      output_df <- data.frame(
        species = single_category$species,
        trait = "all_traits",
  ...
  species_traits_long_categories$has_trait <-
    !is.na(species_traits_long_categories$trait_value) &
    species_traits_long_categories$trait_value != "NaN"
  ...
  common_colnames <- intersect(
    colnames(species_traits_long_categories),
    colnames(number_species_per_trait)
  )

  species_traits_long_categories <- merge(
    species_traits_long_categories,
    number_species_per_trait,
    by.x = common_colnames,
    by.y = common_colnames
  )
  ...
  if (!is.null(species_categories)) {
    category_facet <- ggplot2::facet_wrap(
      ggplot2::vars(
        !!rlang::sym(colnames(species_categories)[[2]])
      ),
      scales = "free"
    )
  } else {
  ...
  ggplot2::ggplot(
    ...
    ggplot2::aes(
      factor(
        .data$trait_label,
        levels = unique(number_species_per_trait$trait_label)
      ),
      factor(
        .data$species,
        levels = unique(number_trait_per_species$species)
      )
    )
  ) +
