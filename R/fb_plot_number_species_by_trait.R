...
fb_plot_number_species_by_trait <- function(
  ...
  number_species_per_trait <- lapply(
    ...
    function(x) {
      ...
      trait_coverage$trait <- factor(
        ...
        levels = rev(trait_coverage$trait)
      )

      trait_coverage[["prop_species"]] <- trait_coverage[["n_species"]] /
        nrow(x)
  ...
  if (!is.null(threshold_species_proportion)) {
    vline_annotation <- list(
      ...
      ggplot2::annotate(
        "text",
        x = threshold_species_proportion * n_species,
        y = 0.95,
        hjust = 1.1,
        color = "darkred",
        label = paste0(
          "(n = ",
          round(threshold_species_proportion * n_species),
          ")\n(p = ",
  ...
  ggplot2::ggplot(
    ...
    ggplot2::aes(.data$n_species, .data$trait)
  ) +
    ggplot2::geom_point() +
