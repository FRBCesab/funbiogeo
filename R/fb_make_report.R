...
fb_make_report <- function(
  ...
  # Ask user for creating copy of datasets

  con <- getOption("fb_ask_options", stdin())
  ...
  if (!(answer %in% c("y", "n"))) {
    stop("Please answer '[y]es' or '[n]o'", call. = FALSE)
  }
  ...
  if (file.exists(path_rmd) && !overwrite) {
    stop(
      "The file '",
      path_rmd,
      "' already exists. If you want to replace it, ",
      "use 'overwrite = TRUE'.",
      call. = FALSE
    )
  }
  ...
  if (file.exists(filename) && !overwrite) {
    stop(
      "The file '",
      filename,
      "' already exists. If you want to replace ",
      "it, use 'overwrite = TRUE'.",
      call. = FALSE
    )
  }
  ...
  invisible(
    file.copy(
      system.file(
        file.path("templates", "template_report.Rmd"),
        package = "funbiogeo"
      ),
      path_rmd,
      overwrite = TRUE
