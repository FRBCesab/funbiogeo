...
fb_format_site_species <- function(
  data,
  site,
  species,
  value,
  ...
  if (!(species %in% colnames(data))) {
    stop(
      paste0("The column '", species, "' is absent from 'data'"),
      call. = FALSE
    )
  }
  ...
  if (!(value %in% colnames(data))) {
    stop(
      paste0("The column '", value, "' is absent from 'data'"),
      call. = FALSE
    )
  }

  if (!is.numeric(data[, value])) {
    stop(paste0("The column '", value, "' is must be a numeric"), call. = FALSE)
  }
